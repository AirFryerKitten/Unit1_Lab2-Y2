...
def select(rats, pups):
  ...
  if smallMale > smallFemale:
    smallest = smallFemale
  elif smallMale < smallFemale:
    smallest = smallMale
  else:
    smallest = smallMale
...
def rat_test():
  ...
  while goalStatus == False:
    ...
    rats, smallest, largest = select(rats,pups)
    pups = breed(rats)
    pups = mutate(pups)
    listMeans.append(mean)
    listSmall.append(smallest)
    listBig.append(largest)
    generations += 1
  ...
  with open("smallInfo.txt",'w') as openFile:
    for small in listSmall:
      openFile.write(f"{str(small)} ")
  with open("bigInfo.txt",'w') as openFile:
    for largest in listBig:
      openFile.write(f"{str(largest)} ")
  with open("average.txt",'w') as openFile:
    for mean in listMeans:
      openFile.write(f"{str(mean)} ")
